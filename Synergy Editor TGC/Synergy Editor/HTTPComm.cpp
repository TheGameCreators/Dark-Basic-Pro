...
void ReportError( void )
{
	...
	if ( g_bExtraReporting )
	{
		if ( dwErr==ERROR_INTERNET_EXTENDED_ERROR )
		{
			...
			DWORD dwErrMsg = 0;
			DWORD dwSize = 10240;
			InternetGetLastResponseInfoA ( &dwErrMsg, err, &dwSize );
		...
		else
		{
			...
			if ( lpMsgBuf )
			{
				strcat ( g_pFeedbackAreaString, (LPSTR)lpMsgBuf );
				strcat ( g_pFeedbackAreaString, "\r\n" );
				LocalFree( lpMsgBuf );
			}
			else
			{
				char num[256];
				sprintf(num,"Error Code:%d", dwErr );
...
bool HTTP_Connect ( char* url )
{
	...
	if(hInetConnect==NULL)
	{
		...
		else
		{
			...
			if(hInet==NULL)
			{
				hInet = InternetOpenA(	"InternetConnection",
				...
				if(hInet==NULL)
				{
					if ( g_bExtraReporting )
					{
						strcat ( g_pFeedbackAreaString, "could not open connection" );
						strcat ( g_pFeedbackAreaString, "\r\n" );
					}
					ReportError();
				}
			...
			{
				// Internet Connect
				hInetConnect = InternetConnectA(	hInet,
												url,
												wHTTPType,
												"",
												NULL,
				...
				if(hInetConnect==NULL)
				{
					if ( g_bExtraReporting )
					{
						strcat ( g_pFeedbackAreaString, "could not connect to internet" );
						strcat ( g_pFeedbackAreaString, "\r\n" );
					}
					ReportError();
				}
...
LPSTR HTTP_RequestData ( LPSTR pVerb, LPSTR pObjectName, LPSTR pHeaderString, DWORD dwHeaderSize, LPSTR pPostData, DWORD dwPostDataSize )
{
	...
	// prepare request
	HINTERNET hHttpRequest = HttpOpenRequestA ( hInetConnect, pVerb, pObjectName, "HTTP/1.1", "The Agent", NULL, dwFlag, 0 );

	// send request with header
	BOOL bSendResult = HttpSendRequestA ( hHttpRequest, pHeaderString, dwHeaderSize, pPostData, dwPostDataSize );
	if ( bSendResult )
	{
		// reading all data
		do
		{
			...
			if (!InternetQueryDataAvailable(hHttpRequest,&dwSize,0,0))
			{
				...
				if ( g_bExtraReporting )
				{
					strcat ( g_pFeedbackAreaString, "error obtaining chunk of data" );
					strcat ( g_pFeedbackAreaString, "\r\n" );
				}
				ReportError ();
				break;
	...
	else
	{
		...
		if ( g_bExtraReporting )
		{
			strcat ( g_pFeedbackAreaString, "HttpSendRequest failed" );
			strcat ( g_pFeedbackAreaString, "\r\n" );
		}
		ReportError();
	}
