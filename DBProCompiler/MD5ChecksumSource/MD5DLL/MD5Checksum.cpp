...
// DLL Main and Exports

/* MSVC6 does not need this when using MFC shared
...
__declspec ( dllexport ) LPSTR GetMD5 ( BYTE* pMangled, UINT uSize )
{
	CString pResult = CMD5Checksum::GetMD5 ( pMangled, uSize );
	LPSTR pLookSeeString = pResult.GetBuffer((int)256);
	LPSTR pOut = NULL;
	pOut = new char [ strlen(pLookSeeString)+1 ];
	strcpy ( pOut, pLookSeeString );
	return pOut;
}

__declspec ( dllexport ) void FreeMD5 ( LPSTR pFreeString )
{
	if ( pFreeString )
	{
		delete pFreeString;
