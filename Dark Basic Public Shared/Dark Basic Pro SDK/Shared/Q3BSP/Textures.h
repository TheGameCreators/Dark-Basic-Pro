...
extern struct _Textures
{
	...
	void NotFound_Resize ( int count )
	{
		if ( count > NotFound_size )
		{
			...
			for ( int x = NotFound_size; x < count; x++ )
				NotFound_FileName [ x ] = NULL;
	...
	struct _cache
	{
		...
		void Resize ( int num )
		{
			...
			else if ( size - num > 100 )
			{
				...
				{ 
					SAFE_DELETE ( entry [ x ].filename );
					SAFE_DELETE ( entry [ x ].path );
					SAFE_DELETE ( entry [ x ].fullname );
					SAFE_DELETE ( entry [ x ].resource );
				}
