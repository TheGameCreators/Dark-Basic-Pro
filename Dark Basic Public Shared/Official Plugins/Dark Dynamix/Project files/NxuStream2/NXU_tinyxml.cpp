...
bool TiXmlDocument::LoadFile(TiXmlEncoding encoding)
{
	...
	if (buf.buffer &&	LoadFile(buf.buffer, 0, 0, encoding))
	{
		return true;
	}
...
bool TiXmlDocument::SaveFile(NXU_FILE	*fp)const
{
	if (useMicrosoftBOM)
	{
		...
		nxu_fputc(TIXML_UTF_LEAD_0,	fp);
		nxu_fputc(TIXML_UTF_LEAD_1,	fp);
		nxu_fputc(TIXML_UTF_LEAD_2,	fp);
	}
	Print(fp,	0);
	return true;
}
