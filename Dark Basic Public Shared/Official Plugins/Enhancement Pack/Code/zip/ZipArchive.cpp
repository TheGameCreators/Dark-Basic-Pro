...
//////////////////////////////////////////////////////////////////////
TCHAR CZipArchive::m_gszCopyright[] = {_T("")};
...
bool CZipArchive::Open(LPCTSTR szPathName, int iMode, int iVolumeSize)
{
	if (!IsClosed())
	{
	...
	m_iFileOpened = nothing;
	if ( !m_storage.Open(szPathName, iMode, iVolumeSize))
		return false;
	m_centralDir.Init();
...
bool CZipArchive::ThrowError(int err)
{
// leechange - added BOOL and remove this which just bombs the app!
...
bool CZipArchive::OpenFile(WORD uIndex)
{
	...
	m_info.Init();
	if ( !m_centralDir.OpenFile(uIndex) )
		return false;
	...
	if (CurrentFile()->IsEncrypted())
	{
		...
		if (!CryptCheck())
		{
			return ThrowError(ZIP_BADPASSWORD);
		}
	...
	if ((uMethod != 0) &&(uMethod != Z_DEFLATED))
		return ThrowError(ZIP_BADZIPFILE);
...
bool CZipArchive::OpenNewFile(CZipFileHeader & header, int iLevel, LPCTSTR lpszFilePath)
{
	...
	if (!CurrentFile()->PrepareData(iLevel, m_storage.IsSpanMode() == 1, bEncrypted))
		return ThrowError(ZIP_TOOLONGFILENAME);
                              ...
                              DWORD nBufSize)
{
	...
	*/
	...
	else
	{
		...
		///////
		//CFileStatus status;
		//f.GetStatus( szFile, status );
		//status.m_attribute = 0;
		//f.SetStatus( szFile, status );
...
DWORD CZipArchive::RemovePackedFile(DWORD uStartOffset, DWORD uEndOffset)
{
	...
	if (uTotalToWrite != TotalWritten)
		return ThrowError(CZipException::generic);
...
bool CZipArchive::TestFile(WORD uIndex, ZIPCALLBACKFUN pCallback, void* pUserData, DWORD uBufSize)
{
	...
	if (IsFileDirectory(uIndex))
	{
		...
		if ((iSize != 0 || iSize != pHeader->m_uUncomprSize)
			...
			return ThrowError(CZipException::dirWithSize);
		return true;
	}
	else
	{
