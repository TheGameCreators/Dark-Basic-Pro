...
struct CZipAutoHandle
{
	...
	{
		m_hFileMap = CreateFileMapping(hFile, NULL, PAGE_READWRITE,0, 0, _T("File Blocks"));
		if (!m_hFileMap)
			return false;
...
DWORD CZipCentralDir::Locate()
{
	...
	DWORD uFileSize = (ULONGLONG) m_pStorage->m_pFile->GetLength();
...
bool CZipCentralDir::ThrowError(int err)
{
// this just bombs any app, added graceful BOOL
...
bool CZipCentralDir::OpenFile(WORD uIndex)
{
